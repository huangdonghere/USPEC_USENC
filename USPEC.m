...
function labels = USPEC(fea, Ks, p, Knn, maxTcutKmIters, cntTcutKmReps)
...
for i = 1:cntRepCls
    [~,nearestRepInRpFeaIdx(minCenterIdxs==i)] = min(pdist2(fea(minCenterIdxs==i,:),RpFea(repClsLabel==i,:)),[],2);
...
for i = 1:p
    ...
    RpFeaKnnDist(nearestRepInRpFeaIdx==i,:) = pdist2(fea(nearestRepInRpFeaIdx==i,:), RpFea(RpFeaKnnIdx(i,:),:));
end
clear fea RpFea
...
[~, RpFea] = litekmeans(bigRpFea,pSize,'MaxIter',20);
